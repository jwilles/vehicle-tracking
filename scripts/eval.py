...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path, max_truncation=0, min_height=25, max_occlusion=2, mail=None, cls="car", eval_3diou=True, eval_2diou=False):
            ...
            for line in f:
                ...
                try:
                    ...
                    if id_frame in id_frame_cache and not loading_groundtruth:
                        self.mail.msg("track ids are not unique for sequence %d: frame %d" %
                                      (seq, t_data.frame))
                        self.mail.msg(
    ...
    def getThresholds(self, scores, num_gt, num_sample_pts=num_sample_pts):
        ...
        # may not be 11 due to not fully recall the results, all the results point
        # has zero precision
    ...
    def compute3rdPartyMetrics(self, threshold=-10000):
        ...
        # groundtruth and tracker contain lists for every single frame containing
        # lists of KITTI format detections
        fr, ids = 0, 0
            ...
            for f in range(len(seq_gt)):        # go through each frame
                ...
                for i, tt in enumerate(this_ids[0]):
                    ...
                    if tt in last_ids[0]:
                        ...
                        # id in current tracker corresponding to the gt tt
                        tid = this_ids[1][i]
        ...
        if self.n_gt != 0:
            if self.id_switches == 0:
                ...
                self.MOTAL = 1 - (self.fn + self.fp +
                                  math.log10(self.id_switches)) / float(self.n_gt)
        else:
    ...
    def createSummary_details(self):
        ...
        # summary += self.printEntry("Ground Truth Objects (Total) per Sequence",
        # self.n_gts) + "\n"
        summary += self.printEntry("Ignored Ground Truth Objects", self.n_igt) + "\n"
        # summary += self.printEntry("Ignored Ground Truth Objects per Sequence",
        # self.n_igts) + "\n"
        ...
        # summary += "\n"
        # summary += self.printEntry("Ignored Tracker Objects with Associated
        # Ignored Ground Truth Objects", self.n_igttr) + "\n"
        summary += "=" * 80
    ...
    def saveToStats(self, dump, threshold=None):
        ...
        if threshold is None:
            ...
            summary = self.createSummary_simple(threshold)
...
class stat:
    ...
    def plot_over_recall(self, data_list, title, y_name, save_path):
        ...
        if y_name in ['MOTA', 'F1']:
            ...
            plt.text(self.recall_list[max_ind] - 0.05, data_list[max_ind] +
                     0.03, '%.2f' % (data_list[max_ind]), fontsize=20)
...
def evaluate(result_sha, mail, eval_3diou, eval_2diou):
    ...
    for c in ("car", "pedestrian", "cyclist"):
        # for c in ("car"):
        e = trackingEvaluation(gt_path=gt_path, t_sha=result_sha, mail=mail, cls=c,
        ...
        for threshold_tmp in threshold_list:
            ...
            data_tmp['mota'], data_tmp['motp'], data_tmp['moda'], data_tmp['modp'], data_tmp['precision'], \
                data_tmp['F1'], data_tmp['fp'], data_tmp['fn'], data_tmp[
                    'recall'] = e.MOTA, e.MOTP, e.MODA, e.MODP, e.precision, e.F1, e.fp, e.fn, e.recall
